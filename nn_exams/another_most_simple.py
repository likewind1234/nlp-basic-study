...
cross_entropy = -tf.reduce_mean(y_ * tf.log(tf.clip_by_value(y, 1e-10, 1.0))) # https://www.cnblogs.com/hypnus-ly/p/8047214.html
train_step = tf.train.AdamOptimizer(0.001).minimize(cross_entropy) # 固定模式，定义了优化方法和损失函数，由此定义了反向传播算法，此时图定义出来了
# 定义损失函数和反向传播算法
